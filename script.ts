function program() {
  ...
  class Color {
    ...
    model: string;
    cachedValue: any;
    constructor(c1: number, c2: number, c3: number, input1?: number | string, input2?: string) {
      let alpha, model;
  ...
  function regularPolyVerts(x: number, y: number, radius: number, sideCount: number) {
    const vertices = [];
  ...
  class Shape {
    ...
    constructor(
      ...
      if (this.type === "Polygon") {
        if (vertices === undefined) throw new Error("Polygons require vertices");
  ...
  class Hitbox extends Shape {
    ...
    constructor(shape: Shape, base: Base, rho?: number) {
      ...
      if (this.type === "Circle") {
        ...
        if (this.windingOrder === "CW") loopedVertices = this.vertices.slice().reverse();
    ...
    narrowUpdate(base: Base) {
      ...
      this.transformedVertices = this.vertices.map((vertex) =>
        vertex.copy().rotate(base.dir).add(base.position)
      );
    ...
    closestPointToCentroidOf(base: Base): Vector {
      ...
      this.transformedVertices.forEach((vertexA: Vector, index: number, vertices: Vector[]) => {
  ...
  class Base {
    ...
    constructor(position: Vector, dir: number, shape: Shape, velocity?: Vector) {
      this.id = newid();
      this.indices = [];
    ...
    handleCollision(data: { colliding: boolean; axis: Vector; overlap: number; baseA: Base; baseB: Base }) {
      ...
      if (collisionType === "Polygon-Polygon") {
        ...
        */
        ...
        if (contactPoints.length === 0) contactPoints.push(baseA.hitbox.closestPointToCentroidOf(baseB));
        ...
        contactPoints.push(
          baseA.position.copy().add(centroidAToCentroidB.copy().multiply(baseA.hitbox.radius))
        );
      }
  ...
  for (let i = 0; i < insertCount; i++) {
    ...
    if (random(0, 5) > 4) {
      ...
      shape = newPolygon(
        polyVerts(-hWidth, -hHeight, hWidth, -hHeight, hWidth, hHeight, -hWidth, hHeight),
        new Color(255, 0, 0)
      );
    }
    boxes.push(
      new Base(
        new Vector(random(-canvasHalfWidth, canvasHalfWidth), random(-canvasHalfHeight, canvasHalfHeight)),
        random(0, 2 * PI),
        shape
      )
    );
  ...
  if (insertCount === 0)
    boxes.push(
      ...
      new Base(new Vector(0, 100), 1.5, newPolygon(regularPolyVerts(0, 0, 35, 5), new Color(255, 0, 0))),
      new Base(
        new Vector(0, 200),
        1.2,
        newPolygon(polyVerts(-25, -50, 25, -50, 25, 50, -25, 50), new Color(255, 0, 0))
      )
    );
